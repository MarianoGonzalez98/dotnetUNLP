/* 
Completarla y agregar dos indizadores de sólo lectura.
Un índice entero que permite acceder a las personas de la lista por número de documento. 
Por ejemplo p=lista[30456345] devuelve el objeto Persona que tiene DNI=30456345 o null en caso que no exista en la lista.
Un índice de tipo char que devuelve un List<string> con todos los nombres de las personas de la lista que comienzan con el carácter pasado como índice.
 */
class ListaDePersonas
{
    private List<Persona> _lista;
    
    public ListaDePersonas()
    {
        this._lista= new List<Persona>();
    }
    public void Agregar(Persona p)
    {
        this._lista.Add(p);
    }

    //indizadores
    public Persona? this[int i]
    {
        get
        {
            Persona? p = _lista.Find(x => x.Dni == i);
            return p;
        }
    }
    
    public List<string> this[char c]
    {
        get
        {
            List<string> lista = new List<string>();
            for (int i = 0; i < this._lista.Count; i++)
            {
                if (this._lista[i]?.Nombre?[0]==c)
                {
                    lista.Add(this._lista[i].Nombre);

                }
            }
            return lista;
        }
    }

}